# Name: 02b_crossSectional12m.R
# Auth: umar.niazi@kcl.ac.uk
# Date: 23/02/2017
# Desc: relationships of diversity with covariates - response is chao1
...
# remove shannonmean and keep chao
...
ivScore
# remove lowest scores 2 scores
length(ivScore)
ivScore = ivScore[1:57]
tail(ivScore)
...
write.csv(dfUnivariate, file='mergedDataSet/Temp/uni_chao.csv')
...
dfData$abxMonth = factor(ifelse(dfData$abxMonth == 1, 'Yes', 'No'))
dfData$abx3Months = factor(ifelse(dfData$abx3Months == 1, 'Yes', 'No'))
fm01 = lm(Chao ~ eczema12m + ., data=dfData)
summary(fm01)
...
cvRemove = c('catsCount3m', 'dogsCount3m ', 
             'Ige12m0.35')
...
fm01.b = lm(Chao ~ eczema12m + iga + caesarean + numSiblings3m + abxMonth + countryside + calp + bdef+ 
              weight12m + intervention  , data=dfData)
